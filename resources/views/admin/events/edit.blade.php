...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <h2 class="text-2xl font-semibold mb-6">Редактирование мероприятия: {{ $event->title }}</h2>
        ...
        <form action="{{ route('admin.events.update', $event) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <textarea name="additional_info" id="additional_info" rows="3" 
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('additional_info') border-red-500 @enderror">{{ old('additional_info', $event->additional_info) }}</textarea>
            ...
            <div class="mb-4">
                ...
                <select name="status" id="status" 
                    ...
                    required>
                    <option value="published" {{ old('status', $event->status) == 'published' ? 'selected' : '' }}>Опубликовано</option>
                    <option value="draft" {{ old('status', $event->status) == 'draft' ? 'selected' : '' }}>Черновик</option>
            ...
            <div class="mb-6">
                ...
                @if($event->image)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $event->image) }}" alt="{{ $event->title }}" class="h-40 object-cover rounded">
                    </div>
                @endif
                <input type="file" name="image" id="image" 
                    class="w-full @error('image') border-red-500 @enderror">
                <p class="text-gray-500 text-xs mt-1">Рекомендуемый размер: 1200x630 пикселей. Максимальный размер: 2MB. Оставьте пустым, чтобы сохранить текущее изображение.</p>
                @error('image')
                    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                @enderror
            </div>
